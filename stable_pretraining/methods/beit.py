...
def patch_kmeans_tokenizer(vocab_size: int, patch_size: int, in_channels: int = 3):
    """Return a dummy hashing tokenizer for BEiT smoke tests.

    Hashes flattened patches into ``vocab_size`` buckets via a fixed random
    projection. Useful for end-to-end smoke tests only — replace with a
    DALL-E or VQ-VAE tokenizer for real training.
    """
    proj = None
